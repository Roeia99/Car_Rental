...
<head>
    <title>Advanced Search Car</title>
    ...
    <script src="Advanced_Car.js"></script>
    ...
    <div class="d-flex justify-content-center h-100">
        <div class="card" style="width: 90rem;height: 40rem;">
            ...
            <div class="card-header">
                <h3>Advanced Car Search</h3>
            </div>
            <div class="card-body">
                <form id="filter-form" action="Advanced_Filter_Car.php" method="POST">
                            ...
                            <div class="col-sm-3">
                                ...
                                <div class="form-group">
                                    <select class="custom-select mr-sm-2"
                                        ...
                                        <option selected value= "">(Office)</option>
                                              <?php
                                        $connection = mysqli_connect('localhost', 'root', '', 'car_rental_system');
                                        $sql = "SELECT distinct c.off_id `off_id` FROM `car` c ";
                                        $query = mysqli_query($connection, $sql);
                                        while ($row = mysqli_fetch_assoc($query)) {
                                            echo "<option> $row[off_id] </option>";
                                        }
                                        mysqli_close($connection);
                                        ?>
                                    </select>
                                ...
                                <div class="form-group">
                                    <select class="custom-select mr-sm-2"
                                            id="price-filter"
                                            style="width: 18rem;">
                                        <option selected value= "">(Price per Day)</option>
                                              <?php
                                        $connection = mysqli_connect('localhost', 'root', '', 'car_rental_system');
                                        $sql = "SELECT distinct c.price_per_day `price_per_day` FROM `car` c ";
                                        $query = mysqli_query($connection, $sql);
                                        while ($row = mysqli_fetch_assoc($query)) {
                                            echo "<option> $row[price_per_day] </option>";
                                        }
                                        mysqli_close($connection);
                                        ?>
                                    </select>
                                        ...
                                        <div class="container-fluid">
                                            <table id="infoTable"
                                                ...
                                                <tr>
                                                    ...
                                                    <th class="col-xs-3">Office ID</th>
                                                    <th class="col-xs-6">Price Per Day</th>
                                                </tr>
                                                </thead>
                                            </table>
                                        </div>
